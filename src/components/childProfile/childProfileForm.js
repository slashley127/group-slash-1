...
class childProfileInput extends Component{
    ...
    async componentDidMount() {
        ...
        if (id !== 'new') {
            const child = await(await fetch(`/childprofile/${id}`)).json();
    ...
    async handleSubmit(e) {
        ...
        if (!item.id) {
            ...
            await fetch(`/childprofile/${item.id}`, {
                method: "PUT",
                headers: {
                    'Accept': "application/json",
                    'Content-Type': "application/json"
        ...
        this.props.navigation("/childprofile");
    }
...
export default withParams(withNavigateHook(childProfileInput));